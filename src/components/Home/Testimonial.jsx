...
// Removed import { FaStar } from 'react-icons/fa'; to resolve the error

const Testimonial = () => {
  const testimonials = [
    {
      id: 1,
      image: "https://placehold.co/100x100/F0F8FF/000?text=Student+1", // Replace with actual image URL
      name: "Rohini",
    ...
    {
      id: 2,
      image: "https://placehold.co/100x100/F5FFFA/000?text=Student+2", // Replace with actual image URL
      name: "Priya",
    ...
    {
      id: 3,
      image: "https://placehold.co/100x100/FFF8DC/000?text=Student+3", // Replace with actual image URL
      name: "Anjali",
    ...
    {
      id: 4,
      image: "https://placehold.co/100x100/F0FFFF/000?text=Student+4", // Replace with actual image URL
      name: "Rahul",
      role: "Online Learner",
      rating: 5,
    ...
    {
      id: 5,
      image: "https://placehold.co/100x100/F8F8FF/000?text=Student+5", // Replace with actual image URL
      name: "Deepa",
      role: "Career Changer",
      rating: 4,
  ...
  // This logic displays the current, next, and next-next testimonials
  // It wraps around using modulo operator
  const getDisplayedTestimonials = () => {
    const displayed = [];
    for (let i = 0; i < 5; i++) { // Display 5 images
    ...
    <section className="py-16 px-4 bg-white font-family overflow-hidden">
      <div className="container mx-auto text-center">
        ...
        <div className="relative flex items-center justify-center mb-12">
          ...
          <button
            onClick={handlePrev}
            className="absolute left-0 lg:left-40 p-3 rounded-full bg-gray-100 hover:bg-gray-200 shadow-md transition-colors duration-200 z-10"
            ...
            {displayedTestimonials.map((testimonial, index) => (
              <div
                ...
                className={`flex-shrink-0 transition-all duration-300 ease-in-out ${
                  index === 0 ? 'scale-75 opacity-50' : // Leftmost (hidden/small)
                  index === 1 ? 'scale-90 opacity-75' : // Second from left (smaller)
                  index === 2 ? 'scale-100 opacity-100 z-10' : // Center (active)
                  index === 3 ? 'scale-90 opacity-75' : // Second from right (smaller)
                  'scale-75 opacity-50' // Rightmost (hidden/small)
                }`}
              >
                <img
                  ...
                  className={`rounded-full object-cover object-center border-4 ${
                    index === 2 ? 'border-green-500' : 'border-transparent' // Green border for active image
                  }`}
                  style={{ width: index === 2 ? '120px' : '90px', height: index === 2 ? '120px' : '90px' }} // Dynamic size
                  onError={(e) => { e.target.onerror = null; e.target.src = 'https://placehold.co/100x100/E0F2F7/000?text=User'; }}
                />
              </div>
            ))}
          </div>
          ...
          <button
            onClick={handleNext}
            className="absolute right-0 lg:right-40 p-3 rounded-full bg-gray-100 hover:bg-gray-200 shadow-md transition-colors duration-200 z-10"
        ...
        {activeTestimonial && (
          <div className="max-w-3xl mx-auto">
            ...
            <div className="flex justify-center mb-4">
              {[...Array(5)].map((_, i) => (
                <StarIcon // Using the custom StarIcon component
                  key={i}
                  className={i < activeTestimonial.rating ? 'text-green-500 w-5 h-5' : 'text-gray-300 w-5 h-5'} // Apply Tailwind classes
                />
              ))}
            </div>
